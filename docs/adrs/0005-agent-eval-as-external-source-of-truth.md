...
**This ADR describes the intended target architecture, not something currently deployable end-to-end.** `agent-eval/infra/` today is a bare local Postgres `docker-compose.yml` - no Cloud Run deployment, no Dockerfile, nothing reachable outside a developer's machine - and neither this platform nor `agent-eval` has any service-to-service authentication yet. Real Cloud Tasks → deployed-`agent-eval` traffic therefore has two hard prerequisites this repo does not unilaterally control: `agent-eval` being deployed somewhere reachable, and some authenticated channel existing between the two services. These are made an explicit gate on Phase 3 (`docs/roadmap.md`), not an assumption this ADR quietly relies on - see `docs/evaluation-and-promotion.md`'s "Prerequisite" section. Until both are true, this architecture is implemented and exercised against a local/dev `agent-eval` instance over an unauthenticated connection, which proves the integration shape but is explicitly not a production-safe deployment.
