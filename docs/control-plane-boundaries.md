...
And the Incident Operations MCP server's `create_ticket` approval gate is enforced only by prompt convention today, not by the server (confirmed by inspection, and explicitly flagged as a known limitation in that codebase's own ADR-013). This platform does **not** silently assume that gate is real. It records `MCPTool.requires_approval = true` as a **governance declaration** — a statement of what *should* be true — and treats actually enforcing it at call time as squarely the execution plane's job. See [`mcp-governance.md`](mcp-governance.md) for the authorization-vs-approval distinction this forces.
