...
**Phase 3 made this boundary real, not just designed.** `agent-eval` is now deployed to Cloud Run ([ADR-0016](adrs/0016-agent-eval-deployment-decision.md)) and this platform's evaluation worker calls its real API over the network, with real authentication - live-verified end to end against the real `incident-investigator` agent, including a real gate failure caused by real evaluation data (`docs/phase-notes/phase-3.md`). The boundary held under real conditions exactly as designed: this platform never touched `agent-eval`'s database, never recomputed a score, and correctly treated one evaluator's real inapplicability to a real dataset as evidence to act on (fix the policy) rather than a bug to route around.
