...
### Agent
`id, name (unique), team_id, description, is_representative_data, created_at`
...
**`is_representative_data` (boolean, default `false`) - added during Phase 1 implementation, not present in this doc's original Phase 0 version.** [`product-overview.md`](product-overview.md) requires that the seeded Customer Support Agent and Release Risk Agent be "clearly identified as representative platform data, not falsely presented as live systems" on every surface that displays them, but Phase 0's domain model had no field for a UI to key that labeling off of - an oversight caught only once the seed script and a real list endpoint were actually built and it became clear there was nothing to check. `false` for the real integration (`incident-investigator`), `true` for the two seeded agents.
...
### EvaluationPolicy
`id, name, version, required_evaluator_keys (jsonb), thresholds (jsonb), max_new_regressions (int), zero_failure_tags (text[]), min_completion_rate (numeric), dataset_key, created_by, created_at`
Versioned and immutable per `(name, version)` - as of Phase 3, enforced at the DB level (`UPDATE`/`DELETE` revoked for `agent_platform_app`, matching `AgentVersion`/`SkillVersion`), not just by convention. See [ADR-0015](adrs/0015-evaluation-policy-immutability.md). `required_evaluator_keys` pins a specific version per key (`{"grounding_judge": "v1"}`), not just presence. `max_new_regressions` generalizes Phase 0's `zero_new_regressions` boolean to a count (0 = the original zero-tolerance default). `name` resolves to "the agent this policy governs" by matching `Agent.name` - `get_current_policy_for_agent` always resolves to the newest `(name, version)` row. See [`evaluation-and-promotion.md`](evaluation-and-promotion.md) for the full gate/threshold structure.

### EvaluationRunReference
`id, agent_version_id, evaluation_policy_id, external_run_id?, external_agent_version_id, external_dataset_id?, external_evaluator_ids (jsonb)?, requested_by, requested_at, status, dataset_snapshot_hash?, dataset_case_set_fingerprint?, evaluator_versions (jsonb)?, dimension_stats (jsonb)?, n_cases_total?, n_cases_success?, n_cases_error?, error_message?, capability_grant_snapshot_hash?, capability_grant_snapshot (jsonb)?, idempotency_key?, completed_at?, fetched_at?`
A local pointer to a run that actually executed in `agent-eval`. `external_run_id` is `agent-eval`'s own run id; `external_agent_version_id`/`external_dataset_id`/`external_evaluator_ids` are what was actually submitted, resolved from the pinned `EvaluationPolicy` at request time. `dimension_stats`/`n_cases_*` are summary evidence only - this platform never stores case-level results, those stay in `agent-eval` and are linked to via `external_run_id`. `capability_grant_snapshot_hash`/`capability_grant_snapshot` are ADR-0014's evidence fingerprint, taken at request time (see `app/services/evidence_snapshot.py`). `dataset_case_set_fingerprint` is a deliberately-weaker, distinctly-named proxy for dataset identity - see [`evaluation-and-promotion.md`](evaluation-and-promotion.md#the-dataset-fingerprint-limitation) for exactly what it can't detect. `idempotency_key` (unique when set) lets a retried evaluation-request call return the existing reference instead of creating a duplicate.

### EvaluationGateResult
`id, evaluation_run_reference_id, evaluation_policy_id, gate_type, criterion, expected, actual, passed, reason?, evidence_ref (jsonb)?, evaluated_at`
