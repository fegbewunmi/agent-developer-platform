...
| **Cloud Scheduler** | Recurring, not event-triggered, work. Used for the outbox reconciliation sweep (`agent-platform-outbox-sweep`, every 5 minutes) and, since Phase 7, the public demo's reset (`agent-platform-demo-reset`, every 3 hours). | Deployed for both jobs above. **Not** wired for periodic MCP health checks (see "Known gap" below) - `POST /v1/mcp-servers/{id}/health-check` remains a manually/on-demand-triggered real HTTP check, not yet on a schedule. |
...
- Cloud Scheduler: the outbox reconciliation sweep and the public demo reset (Phase 7) - both recurring on a clock, not triggered by a domain event.
