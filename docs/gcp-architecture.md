...
| **Cloud SQL (PostgreSQL)** | Control-plane data of record — agents, versions, manifests, grants, policies, promotions, audit. Relational integrity (foreign keys, the `UNIQUE (agent_id) WHERE stage='production'` partial index, transactional audit writes) is load-bearing here, not incidental — see [`failure-modes.md`](failure-modes.md) and [`audit-model.md`](audit-model.md). |
...
| **Cloud Tasks** | The one place this platform genuinely needs durable, retryable, long-running background execution: invoking `agent-eval`'s synchronous, multi-minute `POST /runs` without blocking an API request. See [`evaluation-and-promotion.md`](evaluation-and-promotion.md). |
