...
`MCPServer`: `name, environment, owner_team_id, connection_ref, health_status, last_health_check_at`. Modeled on the one real MCP server in the organization, `ai-operations/mcp_server` (the FastMCP process itself identifies as `trace-incidents`; this platform registers it under the name `incident-operations`, matching the `mcp.servers[]` reference used throughout `docs/agent-manifest.md`'s example manifest), which is itself a thin `httpx` wrapper over the Incident Investigation Platform's FastAPI backend, configured via a `BACKEND_URL` env var and spawned over stdio per its `.mcp.json`. `connection_ref` stores a pointer to that backing FastAPI service (`http://127.0.0.1:8080` in local dev; the real deployed `ai-ops-api` Cloud Run URL in the deployed environment, since Phase 6 - see below) - the platform never holds a live MCP client connection to the stdio server itself; health is checked out-of-band against the backend it wraps (see below), and tool execution is always initiated by the agent runtime, never by this platform. `connection_ref` is admin-updatable (`PATCH /v1/mcp-servers/{id}/connection`, added Phase 6) - it was originally create-time-only, which left the deployed environment stuck pointing at the local-dev placeholder until this platform's own real production issue forced the fix; see `docs/phase-notes/phase-6.md`.
...
`MCPServer.health_status` is refreshed by a real HTTP call - `POST /v1/mcp-servers/{id}/health-check` calls `GET {connection_ref}/health` (`app/services/mcp.py::check_server_health`) and records `healthy` (200 response), `degraded` (any other response), or `unavailable` (connection error/timeout). Triggered on demand by any authenticated user (open to Viewer+, since health is availability, not authorization - the same reasoning as the "not blocking new grants" rule below); a periodic Cloud Scheduler-triggered version of the same call remains a named, open gap - not built through Phase 6 either, a deliberate scope call rather than an oversight (see `docs/phase-notes/phase-6.md`'s "Known limitations"). Live-verified against the real `ai-operations` backend during Phase 2 (local, `http://127.0.0.1:8080/health`) and again in Phase 6 against the real deployed environment (`connection_ref` repointed at `ai-ops-api`'s real Cloud Run URL, health check returned `healthy`) - a genuine HTTP call each time, never a simulated status flip.
